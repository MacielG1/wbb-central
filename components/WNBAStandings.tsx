...
export default async function WNBAStandings({ currentTeam, variant = 'compact' }: WNBAStandingsProps) {
  ...
  return (
    <div className={`w-full ${isCompact ? 'mx-auto px-2 lg:px-0.5' : 'max-w-5xl mx-auto'}`}>
      <h2 className={`font-bold mb-${isCompact ? '2' : '4'} text-neutral-800 dark:text-neutral-200 ${isCompact ? 'text-xl' : 'text-2xl text-center'}`}>WNBA Standings</h2>
        ...
        <table className={`w-full ${isCompact ? 'table-fixed' : 'table-auto'}`}>
          <thead>
            <tr className="bg-neutral-50 dark:bg-neutral-950 border-b border-neutral-200 dark:border-neutral-800">
              <th className={`${isCompact ? 'w-7 px-1 py-2' : 'px-2 py-3'} text-center text-neutral-600 dark:text-neutral-400 font-medium text-sm`}>{isCompact ? '#' : 'Rank'}</th>
              <th className={`${isCompact ? 'px-2 py-2' : 'px-1 py-3'} text-left text-neutral-600 dark:text-neutral-400 font-medium text-sm`}>Team</th>
              <th className={`${isCompact ? 'w-7 px-1 py-2' : 'px-1 py-3'} text-center text-neutral-600 dark:text-neutral-400 font-medium text-sm`}>W</th>
              <th className={`${isCompact ? 'w-7 px-1 py-2' : 'px-1 py-3'} text-center text-neutral-600 dark:text-neutral-400 font-medium text-sm`}>L</th>
              <th className={`${isCompact ? 'w-11 px-1 py-2' : 'px-1 py-3'} text-center text-neutral-600 dark:text-neutral-400 font-medium text-sm whitespace-nowrap`}>PCT</th>
              {!isCompact && <th className="px-1 py-3 text-center text-neutral-600 dark:text-neutral-400 font-medium text-sm whitespace-nowrap">GB</th>}
            </tr>
          </thead>
          <tbody>
            {standingsWithGB.map((team, index) => {
              ...
              return (
                ...
                >
                    ...
                    {isCompact ? (
                      <Link href={`/wnba/${team.id}`} className="flex items-center gap-2">
                        <div className="rounded-full flex items-center justify-center w-6 h-6 overflow-hidden shrink-0 bg-neutral-100 dark:bg-neutral-900">
                          {team.logo && <Image src={team.logo} alt={team.name} className="size-4 object-contain" width={16} height={16} unoptimized />}
                        </div>
                  ...
                  <td className={`${isCompact ? 'px-1 py-2' : 'px-1 py-2'} text-center text-neutral-700 dark:text-neutral-300 tabular-nums ${isCompact ? 'text-sm' : ''}`}>
                    {team.wins}
                  </td>
                  <td className={`${isCompact ? 'px-1 py-2' : 'px-1 py-2'} text-center text-neutral-700 dark:text-neutral-300 tabular-nums ${isCompact ? 'text-sm' : ''}`}>
                    {team.losses}
                  ...
                  {!isCompact && <td className="px-2 py-2 text-center text-neutral-700 dark:text-neutral-300 tabular-nums">{(team as any).gamesBehind || '-'}</td>}
                </tr>
              );
            })}
          </tbody>
