...
export default function NavbarSports() {
  ...
  return (
    <nav className="bg-neutral-950 w-full">
        ...
        <ul className="flex">
          {sports.map((sport) => (
            <li key={sport.name}>
              <Link
                href={sport.path}
                className={`px-6 py-1.5 text-sm font-medium transition-colors flex items-center gap-1.5 hover:bg-neutral-900 text-white ${
                  pathname === sport.path ? 'bg-neutral-900' : ''
                }`}
              >
                {sport.logo}
